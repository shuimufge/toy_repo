...
if input_rule > 3**9 - 1:
    raise AssertionError(
        'Input rule_number=%i, but the Rule Number should be within the range [0, 19682].' %
        input_rule)
...
def get_lookup_table(rule_number):
    ...
    if not isinstance(
            rule_number,
            int) or rule_number >= 3**9 or rule_number < 0:
        raise ValueError(
            "Input rule_number=%i, but it must be an integer between [0, 19682]." %
            rule_number)
...
class eca(object):
    ...
    def __init__(self, rule_number, initial_condition):
        ...
        for i in initial_condition:
            if i not in [0, 1, 2]:
                raise ValueError(
                    "Entries in the initial condition must be within [0,1,2].")
...
def plot_spacetime(
    ...
    plt.imshow(spacetime[start_time:end_time],
               cmap=c_map, interpolation='nearest')
    plt.show()

# %%
# test the codes using the same initial_condition as the codes wihtout
